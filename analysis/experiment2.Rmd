...
<!-- ```{r} -->
<!-- grid.arrange( -->
<!--   (plot(hypothesis(exp2_model, "mu2_treatmentLPS = 0"), plot =F)[[1]]) +  -->
<!--     geom_vline(xintercept = 0, linetype = 2) + coord_cartesian(xlim = c(-6, 6)), -->
<!--   (plot(hypothesis(exp2_model, "mu3_treatmentLPS = 0"), plot =F)[[1]]) +  -->
<!--     geom_vline(xintercept = 0, linetype = 2)+ coord_cartesian(xlim = c(-6, 6)) -->
<!-- ) -->

<!-- hypothesis(exp2_model, c("mu2_treatmentLPS > 0",  -->
<!--                          "mu3_treatmentLPS > 0"), -->
...
```{r}
tableS2 <- get_fixed_effects_with_p_values(exp2_model) %>% 
...
names(tableS2)[3:5] <- c("Est. Error", "Lower 95% CI", "Upper 95% CI")
...
LOR_plot <- LOR %>%
  ...
  geom_vline(xintercept = 0.9068997, linetype = 2, colour = "grey") +
  stat_pointintervalh(aes(colour = outcome, fill = outcome), 
...
stats_table <- rbind(
  plotting_data %>%
    filter(outcome == "Stayed inside") %>%
    spread(treatment, prop) %>%
    mutate(`Absolute difference in % bees exiting the hive` = 100 * (LPS - Ringers),
